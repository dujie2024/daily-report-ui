    ...
    <div className="space-y-6 text-slate-800">
      ...
      <div className="flex items-center justify-between border-b border-slate-100 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">全局掌控海上运动项目、帆船帆板租赁、码头管理及特许商业联营营收绩效</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="bg-gradient-to-br from-emerald-50 to-teal-50/20 p-5 rounded-2xl border border-emerald-100 shadow-sm flex flex-col justify-between">
          ...
          <div className="text-3xl font-black text-slate-800 font-mono mt-4">{qihangData.revenue}</div>
          <span className="text-xs text-emerald-600 font-bold mt-2 flex items-center gap-1">
            <TrendingUp className="w-3.5 h-3.5" />
            <span>同比 {qihangData.revenueTrend} 强劲增长</span>
          </span>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between bg-slate-50 p-3 rounded-xl border border-slate-100">
              ...
              <span className="text-slate-800 font-black font-mono text-sm">{Math.round(qihangData.rawRevenue * 1.15).toLocaleString('en-US')} 元</span>
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                  <div className="bg-emerald-500 h-full" style={{ width: `${((qihangData.rawRevenue / (qihangData.rawRevenue * 1.15)) * 100).toFixed(1)}%` }}></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm mt-6">
        ...
        <div className="mt-3 pt-3 border-t border-slate-100 flex items-center gap-2 text-[11px] text-slate-400">
          ...
          <span>联营商铺数据作为码头服务业态指标。所有分成款项汇入码头商业账户核算。</span>
        </div>
      </div>
    </div>
  );
